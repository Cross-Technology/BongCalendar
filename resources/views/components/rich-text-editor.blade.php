...
{{--
    The one rich-text editor, used by reports, report headers and notes.

    wire:ignore is load-bearing: Quill rewrites this subtree as the user types,
    and letting Livewire morph it would wipe what is being written. The key
    rebuilds it when the caller says the subject changed, so opening a second
    note never shows the first one's text.
--}}
<div wire:ignore
     wire:key="{{ $key }}"
     x-data="quillEditor(@js($model), @js($value), @js($placeholder))"
     x-init="mount()"
